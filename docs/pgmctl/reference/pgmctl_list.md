...
List a resource (alias of `get` for collection-shaped resources)

### Synopsis

Identical to `pgmctl get <resource>` for collection-shaped resources.
Provided so operators don't have to remember whether a resource is a
collection or a singleton.

```
pgmctl list <resource> [flags]
```
