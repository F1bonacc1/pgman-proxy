...
Gracefully promote a specific peer

### Synopsis

Planned switchover to the named target. Cluster-affecting; requires
typed cluster name (or --force --cluster <name>).

```
pgmctl switchover --target <node-id> [flags]
```
...
```
  -h, --help            help for switchover
      --target string   Node id to promote
```
